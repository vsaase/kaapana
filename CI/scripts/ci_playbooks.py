...
def change_openstack_hostname(target_hosts, test_suite_name="Setup Test Server"):
    playbook_path = os.path.join(kaapana_home, "CI/ansible_playbooks/01_set_new_hostname.yaml")
    if not os.path.isfile(playbook_path):
        print("playbook yaml not found.")
        exit(1)
    ...
    return_value, logs = ci_playbook_execute.execute(playbook_path, testsuite=test_suite_name, testname="Change OpenStack Hostname", hosts=target_hosts, extra_vars=extra_vars)
...
def start_install_server_dependencies(target_hosts, remote_username, test_suite_name="Setup Test Server"):
    playbook_path = os.path.join(kaapana_home, "CI/ansible_playbooks/02_install_server_dependencies.yaml")
    if not os.path.isfile(playbook_path):
        print("playbook yaml not found.")
        exit(1)
    ...
    return_value, logs = ci_playbook_execute.execute(playbook_path, testsuite=test_suite_name, testname="Install Server Dependencies", hosts=target_hosts, extra_vars=extra_vars)
    return return_value, logs


def deploy_platform(target_hosts, remote_username, gitlab_username, gitlab_password, platform_name, test_suite_name="Test Platform"):
    playbook_path = os.path.join(
        kaapana_home, "CI/ansible_playbooks/03_deploy_platform.yaml")
    if not os.path.isfile(playbook_path):
        print("playbook yaml not found.")
        exit(1)
...
def delete_platform_deployment(target_hosts, platform_name, test_suite_name="Test Platform", config_file=None):
    ...
    playbook_path = os.path.join(
        kaapana_home, "CI/ansible_playbooks/04_delete_deployment.yaml")
    if not os.path.isfile(playbook_path):
        print("playbook yaml not found.")
        exit(1)
...
def delete_os_instance(username, password, instance_name="kaapana-instance", test_suite_name="Setup Test Server", os_project_name="E230-DKTK-JIP", os_project_id="969831bf53424f1fb318a9c1d98e1941"):
    playbook_path = os.path.join(
    kaapana_home, "CI/ansible_playbooks/06_delete_os_instance.yaml")
    ...
    return_value, logs = ci_playbook_execute.execute(playbook_path, testsuite=test_suite_name, testname="Delete OpenStack CI instance", hosts=["localhost"], extra_vars=extra_vars)
    return return_value, logs
